...
register:
  ...
  motion_register: documentation
---

# Documentation Page

Long-form reference content: legibility and stability. Closest in style to editorial, with optional side navigation. The Shell supplies topbar, footer, the outer offset, and root smooth-scroll; this container is content Sections only, in the fixed order above: title → body.

## Theme

`editorial` only — no theme rotation. The footer's `deep` island belongs to the Shell. Long-form pages use zero radius: structure relies on whitespace and hairlines.
...
- The Shell's `--docs-outer-offset` inset (`foundations/layout.md` §11.2; calibrates to 316px per side at the widest viewport) frames the content region.

### Navigation

- Sticky topbar; optional in-page anchor links honoring the root smooth-scroll.
...
- The outer offset reduces to the standard page gutter below the desktop breakpoint (Shell rule).
