...
A multi-step creation flow that presents two entry paths side by side: a "start blank" prompt column and a "start from a template" gallery, beneath a persistent step indicator. Fills the main pane of an app Shell (default rail-main).

The prompt affordance lives **inside** the body Section's left column (see `shapes/prompt-templates-split.md`) — it is not Shell rail-main-prompt-strip's full-width bottom strip. The two are distinct: the Shell strip pins to the viewport floor of the main column; this container's prompt strip pins near the bottom of its own column within the body Section.
