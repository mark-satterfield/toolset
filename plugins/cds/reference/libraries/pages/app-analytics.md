...
An analytics screen combining filter narrowing with summary tiles and a time-series chart: a filter chip strip at top, a stat-tile grid below it (one row or 2×2, per kpi-summary's arrangement), then a tall chart panel spanning the full width of the Page. Nests in the vacant space of the user's Shell.
