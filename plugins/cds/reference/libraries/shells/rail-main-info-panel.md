---
kind: shell
name: rail-main-info-panel
family: app
aliases: [editor with help panel, workspace with side help, three-column editor shell]
status: stable
sections:
  - { component: left-rail, placement: left pane, notes: "same contract as rail-main" }
panes:
  - { name: rail, width: var(--app-shell-rail-width), collapse: "same ladder as rail-main: icon-only below the tablet breakpoint, drawer in the mobile-narrow range" }
...
# Single side rail + main + right info panel

rail-main plus a persistent right column carrying contextual help, info, or supplementary content for the work in `main`. Each persistent Section of the Shell is deterministic, realized by the named Component.

## Partitioning

- `rail` (left) — same role as in rail-main.
...
- `info-panel` (right, fixed width) — contextual surface that scrolls independently of `main`. It holds a help guide: heading + bulleted tips + a doc deep-link card. The panel is part of the Shell's frame, not part of the page content — it persists across composition states.

## Determinations

- Rail width `--app-shell-rail-width` (matches rail-main). Info panel width `--app-shell-info-panel` (`foundations/layout.md` §11.10). Main pane is fluid between them.
...
Creative tools and editors where the main work surface benefits from persistent, non-modal help (welcome guides, contextual tips, doc deep-links). The help must remain visible while the user works; help that should only appear on demand belongs in rail-main with a popover.

- rail-main-info-panel vs. rail-main: rail-main-info-panel is correct only when contextual help is genuinely persistent and stays relevant across composition states; a one-shot welcome banner belongs inline at the top of rail-main's main, not in an info panel.
- rail-main-info-panel vs. form-sidebar-canvas: rail-main-info-panel's info panel is help, not input. Configuration-that-renders belongs in form-sidebar-canvas's form sidebar.
