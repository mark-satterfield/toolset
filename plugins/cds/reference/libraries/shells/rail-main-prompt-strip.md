...
# Single side rail + main + bottom prompt strip

rail-main plus a bottom-of-viewport prompt or action strip that stays anchored to the viewport floor while main scrolls above it. Each persistent Section of the Shell is deterministic, realized by the named Component.
...
- rail-main-prompt-strip vs. rail-main: the bottom strip earns its space only when the prompt is the keystone of the page. A "send feedback" button does not justify the strip; a project-creation prompt does.
