...
A utility class that neutralizes user-agent and host-cascade styling on interactive primitives (`button`, `input`, `a`, and wrapper `div`s) inside app-surface components, so the component's own contract fully determines rendering. App-surface structural skeletons apply it to wrappers and inner triggers whose visual chrome is owned by the component (filter chip, period picker, toggle switch, and peers).
...
- Components re-declare their own chrome (ground, ring, radius, padding) on top of the reset; the reset guarantees the starting point is the same in a standalone mock and inside a host application's cascade.
