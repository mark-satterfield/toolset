...
- [ ] Text role contrast checked against every theme's surface (`--text-primary` vs `--surface-primary`, etc.) at WCAG AAA for body and AA for tertiary. [scope: both]
...
- [ ] Sample pages visually reviewed in light mode for every Page. [scope: standalone]
- [ ] Sample pages visually reviewed in dark mode for every light-leaning Page. [scope: standalone]
...
14. The top-nav Section's ground matches the page ground exactly. No border. No shadow. [scope: both]
15. Footer columns include language selector and any cookie/consent trigger — not in the top-nav. [scope: both]
...
16. **Topbar height is consistent across the page — the Shell owns it. Do not vary it per Section or per Page.** [scope: both]
...
18. **No page-block override of system-defined geometry.** Spacing, radius, section padding, container width, and a Frame's own surface metrics are owned by the design system and emitted as tokens in the generated stylesheet set (`--{component}-{property}`, `--sp-*`, `--radius-*`, `--section-pad-*`, `--container-*`; see `foundations/layout.md` §11 and `foundations/implementation.md` §8.4). A page-block `<style>` (or inline style) must NOT re-declare one of those tokens to change its value — e.g. setting `--section-pad-main: 40px` in a page block when the scale already carries it. Consume the token instead; if the system lacks a needed dimension, add it to the YAML `geometry:` block, do not override it per page. This rule governs re-declaring a system token, not sizing as such: where a dimension belongs to the Frame hosting a Component rather than to the Component, sizing it in that context is the design, not a violation. [scope: both]
