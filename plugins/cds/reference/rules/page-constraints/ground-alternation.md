...
Section grounds follow a strict, position-determined alternation. The alternation index starts at **1 on the first content Section** in document order and increments by 1 for every following content Section. Shell furniture is not a Section: the topbar carries the page ground via `--nav-bg` (= `surface-primary`) and the footer uses its named theme island. Every Section in the Section Container counts, including T18 heading strips.
