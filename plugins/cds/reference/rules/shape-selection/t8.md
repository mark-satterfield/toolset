...
# Shape selection — T8 Interactive Demo

The pick follows the demo's format: a prompt-to-artifact demo renders as the prompt-artifact shape; a multi-surface demo (one screenshot per surface) tabs the surfaces. Candidates are tried in order (primary, then alternates, then `default`) against the page constraints; if every candidate is rejected, the composer falls back to agent-generated layout and records that in the decisions sidecar.
