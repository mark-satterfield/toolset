...
|-----|------------------------------------|----------------------|
| heading-strip  | Standalone heading strip           | Just an H2 (and optional eyebrow) acting as a section label, no content beneath |
| centered-stack  | Centered text + visual below       | Headline + subhead + CTAs centered; visual (video/screenshot) sits below |
| split-text-media  | Two-column text/visual             | Text + CTAs on one side, single visual on the other |
| centered-affordance  | Centered text + embedded affordance| Centered headline; interactive element (chat input, install snippet, prompt chips) sits where the visual would |
| card-grid  | Static card grid                   | N cards in M columns, identical card structure (icon/title/blurb/optional CTA) |
| tagged-card-grid  | Tagged card grid                   | card-grid plus an eyebrow tag/pill above each card title |
| tabbed-panels  | Tabs with one panel per tab        | Tab strip + a large panel that swaps; each tab gets a full visual |
| alternating-rows  | Alternating image+text rows        | Horizontal rows; image-left/text-right alternates each row |
| card-carousel  | Horizontal carousel                | Card row with prev/next arrows; more items than fit on screen |
| logo-marquee  | Marquee strip                      | Continuously scrolling horizontal row (logos/icons); may duplicate items |
| numbered-steps | Numbered step row                  | 3-column (occasionally more) row with explicit step ordering |
| stacked-quotes | 3-up stacked pull-quotes           | Three quote blocks stacked vertically, no card framing, attribution under each |
| quote-swiper | Quote swiper with logos            | Rotating quote in a viewport; logo carousel beneath rotates in sync |
| feature-quote | Single hero quote                  | One large quote, often with metric, treated as a near-hero element |
| accordion | Accordion list                     | Collapsible items, each header expands to reveal answer; vertical stack |
| pricing-tiers | Tier card row + segment toggle     | Horizontal row of 2-4 tier cards; segment toggle above swaps tier set |
| rate-table | Rate table                         | Rows of usage rates (Input / Output / Cache write / Cache read) in tabular form |
| banner-strip | Banner strip                       | Narrow full-width strip: one headline + 1-2 CTAs, no visual |
| cta-panel | Full-width CTA panel               | Tall full-width band: headline + subhead + CTAs |
| cta-newsletter | CTA panel + newsletter form        | cta-panel plus inline email-capture form with submit and status |
| two-path-fork | Path picker (2-card fork)          | Two large parallel cards representing exclusive paths |
| pictogram-subcards | Pictogram + nested sub-cards       | Animated pictogram as anchor; sub-cards below, each with own "learn more" |
| tag-columns | Pill/tag cloud columns             | 3-5 vertical columns of category pills, each column has a header |
| lead-plus-carousel | Lead card + companion carousel     | One featured large card; secondary cards in a carousel beside it |
| resource-grid | Resource grid with source tags     | Card grid; each card carries a source-type tag (Docs / Blog / Video / etc.) |
| prompt-artifact | Two-column prompt/artifact panel   | Left column = mock user prompt; right column = mock generated artifact |
| install-buttons | Download/install button strip      | Horizontal row of platform-specific install buttons (macOS / Windows / iOS / etc.) |
| footer-grid | Footer navigation grid             | Multi-column nav grid + legal/copyright row |
| sub-hero-split | Sub-hero with video + CTA pair     | Mid-page hero-like restatement: video on one side, headline + CTA on the other |
...
**Determinations**
- CTA count is 1–2; the first is a primary button, the optional second a tertiary/text button beside it (`--sp-1`, 16px, gap). Above 2 CTAs, fall back to centered-affordance or banner-strip.
...
card-grid plus an eyebrow tag/pill above each card title.

- **Slots:** N cards each with eyebrow tag/pill, title, blurb, optional icon, optional CTA.
- **Layout:** grid (inherits card-grid); tag/pill sits above the card title inside each card.
- **Allowed variants:** inherits card-grid variants; adds tag/pill style.
...
- The tag sits `--sp-0-75` (12px) above the title. All other layout inherits card-grid.
...
## pricing-tiers — Tier card row + segment toggle
...
Tall full-width band: headline + subhead + CTAs.
...
## cta-newsletter — CTA panel + newsletter form

cta-panel plus inline email-capture form with submit and status.

- **Slots:** inherits cta-panel slots; adds inline email-capture form (input + submit) and status message slot.
- **Layout:** inherits cta-panel layout; newsletter form sits inline within the band.
- **Allowed variants:** inherits cta-panel variants (CTA count = 1 or 2).
...
## pictogram-subcards — Pictogram + nested sub-cards
...
- Sub-cards follow the card-grid grid (3 columns default, 2 below 700px, 1 below 480px) with the 32px gutter; the "learn more" CTA is a tertiary/text button at the foot of each card. The pictogram-to-sub-card gap is `--sp-4` (52–64px clamp).
...
## lead-plus-carousel — Lead card + companion carousel
...
- The companion carousel inherits card-carousel behavior (snap, prev/next, reduced-motion handling). Below the 700px breakpoint the lead card stacks above the carousel, which becomes finger-scrollable.
...
- **Layout:** grid (card-grid-style) with source tag per card.
- **Allowed variants:** source-tag set is extensible (Docs / Blog / Video / etc.).

**Determinations**
- Inherits the card-grid grid (3 columns default, 2 below 700px, 1 below 480px) and uses the Catalog card spec from `components.md`.
...
## footer-grid — Footer navigation grid
...
## sub-hero-split — Sub-hero with video + CTA pair
...
- **Eyebrows are deny-by-default.** A section-level eyebrow — a short label line above a section's heading — is authorized in exactly two places: the optional eyebrow in heading-strip (the standalone heading strip), and the per-card tag/pill inside tagged-card-grid and resource-grid. No other shape carries a section-level eyebrow. A hero (centered-stack, split-text-media, centered-affordance, sub-hero-split), a card-grid heading (card-grid), a workflow (numbered-steps), a CTA panel (banner-strip, cta-panel, cta-newsletter), and every other shape begin directly with the heading — no kicker above it. When a label genuinely must precede a content unit, model it as its own heading-strip strip rather than decorating an adjacent section's heading.
- **Heading alignment follows the section's content, not a free per-section choice.** Compose each section in one register. **Left register is the default:** the lead heading aligns to the start, matching its content's reading axis — use it whenever the content reads left (accordions accordion, tables rate-table, alternating rows alternating-rows, card/resource grids and tag columns card-grid/tag-columns/resource-grid, and the standalone heading strip heading-strip). **Centered register** centers the lead heading and its content together as one column — reserve it for heroes and full-width editorial / CTA / quote blocks (centered-stack, centered-affordance, stacked-quotes, feature-quote, cta-panel, cta-newsletter) and for short, symmetric content (a centered numbered-step row, a symmetric 2–3 brief-card grid). A centered lead heading must **never** sit above left-reading long-form content (an accordion, a data table, a paragraph stack): when the content reads left, the whole section — heading included — is left. Sub-headings (card titles, column headers, per-row / per-step titles, H3 and below) inherit their section's register and never fight it.
