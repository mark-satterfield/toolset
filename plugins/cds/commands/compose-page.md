...
- Render targets: `assembled` (default, the full page), `container-only` (content without the Shell's persistent Sections), `shell-only` (the frame with an unfilled content slot), `spa` (one Shell, N Section Containers, a client-side switcher), and isolated section/component (wrapped at `--container-marketing-primary` width, `--sp-4` padding, light color-mode default with a top-right toggle).
