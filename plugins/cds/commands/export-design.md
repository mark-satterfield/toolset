---
description: Export the live Configurable Design System as a single DESIGN.md file — the map of colors, typography, geometry, motion, the Building Blocks library, rules, and compliance, written to the DESIGN.md convention for humans and tools alike.
...
3. Run the discovery checklist (resolve `$CUSTOMIZABLE_DESIGN_SYSTEM_ELEMENTS`; resolve the working library).
...
- The emitted `DESIGN.md` follows Google Labs' `design.md` convention (frontmatter tokens + ordered `##` sections), extended with a generation stamp, a Building Blocks section (Components, Shapes, Sections, Pages — plus the statement that Shells are user-composed via `compose-shell` and never shipped), a Rules section, and a How-to-consume section for the CDS model.
- Regenerated, never hand-edited: the emitted header says so. Deterministic — the same elements-YAML meaning + library bytes produce a byte-identical file.
