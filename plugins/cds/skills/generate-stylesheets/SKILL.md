...
5. **Compose `tokens.css`** — CSS custom properties for the whole design system, in one declaration per token (no shorthand). **Only two token families are configurable (bound to elements-YAML values through the role contract in `foundations/overview.md §5`): colors and fonts/typefaces.** Naming follows the YAML's `$conventions` block (e.g., `--color-{palette}-{shade}`, `--role-{key}`, `--type-{key}`, `--space-{key}`). **Every other token is FIXED by the foundations and is NOT YAML-configurable — but it is still required, and `tokens.css` must emit it verbatim from the spec.** In particular, emit from `foundations/layout.md`: the spacing scale (§11.4 `--sp-*`), the border-radius scale (§11.7 `--radius-*`), and the section-padding scale (§11.3 `--section-pad-*`, including the §11.3 narrow-mobile `@media (max-width: 480px)` floor for `--section-pad-main`), using the exact token names `components.md` references and the §11.1 clamp form `clamp(M, calc(M + (X-M) * (100vw - 320px) / 1120), X)`. Omitting these is the failure mode they prevent: `components.css` references `--sp-*`, `--radius-*`, and `--section-pad-*`, so leaving them out of `tokens.css` collapses every component's padding and radius to zero. Every `var(--token)` referenced in `components.css` must be defined in the emitted set — enforced by `test/checks/check_token_coverage.py`.
