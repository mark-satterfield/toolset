...
- **From the library** when relevant (plugin ∪ extensions, per `../../reference/pipeline.md` Library resolution): `libraries/pages/*.md`, `libraries/sections/*.md`, `libraries/shapes/*.md`, `libraries/components/*.md`, and the `rules/shape-selection/` + `rules/page-constraints/` entries they reference.
...
4. **Page and page family.** Which library Page entry (`../../reference/libraries/pages/`) the target corresponds to, if any, and its page family (landing, app, editorial, docs, auth) — needed for type-specific rules: the page family scopes which PageLevelAestheticConstraints apply.
...
3. **Walk the target.** For each rule, check compliance against the target. Record every violation with: rule identifier, observed value, expected value, file path and line (or selector path / pasted-region offset), and a suggested fix that cites the relevant reference section. Adjust rule scope as you go — rules tagged `[scope: app-embedded]` do not apply to a standalone target and vice versa; rules tagged `[scope: both]` apply to either.
   - **Page-block override of system-defined geometry (`compliance.md` §23 #18).** Flag any page-block `<style>` rule or inline `style=` that re-declares a system geometry token (`--sp-*`, `--radius-*`, `--section-pad-*`, `--container-*`, or a `--{component}-{property}` token such as `--topbar-height`) to change its value — the canonical case is a page block setting `--section-pad-main: 40px` while the scale already carries that token. Observed = the page-block declaration; expected = consume the generated token (`var(--section-pad-main)`, `var(--container-marketing-primary)`, …); fix cites `foundations/layout.md` §11 / the owning entry's `sizing`. Two things are **not** this finding: a value the design system does not define (that is `undefined-in-reference`, step 4, fixed by adding it to the YAML `geometry:` block), and a dimension the system deliberately leaves to the hosting Frame rather than to the Component — a mark's height is the hosting Section's `mark-height`, so sizing a mark per Section is the design, not an override.
