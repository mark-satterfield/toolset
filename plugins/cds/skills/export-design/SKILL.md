...
5. **Compose `DESIGN.md`** with this exact shape:
   ...
   **Body** — first line a banner, then the sections in this order:
   ...
   - `## Building Blocks` — the CDS catalog, one subsection per kind (`### Components`, `### Shapes`, `### Sections`, `### Section Containers`, `### Shells`). The section opens with one intro line noting that Elements are the concept-only DOM baseline beneath Components — never configured, never cataloged. Each entry is one row: name, a purpose one-liner, its aliases, and the load-bearing contract — for Components the five contracts in brief (slots, sizing, behavior, accessibility, token bindings), the `### Components` rows being Component Definitions (an on-page Component is an instance of its Definition); for Shapes the slots and `self_contained` flag; for Sections the mode and content contract; for Section Containers the ordered Section list and referenced constraints; for Shells the persistent Sections and panes and accepted content, noting whether the entry is a templated Shell (a Shell Template — persistent Sections pinned around a placeholder Section Container slot) or an instantiated Shell. Each row cites its entry path (e.g. `reference/libraries/components/button.md`) for depth — DESIGN.md names and locates, it does not inline every contract.
   - `## Rules` — `### Shape selection` (per-Section: the signals consulted and that a rule chooses an ordered candidate set, citing `rules/shape-selection/`) and `### Page-Level Aesthetic Constraints` (directory: `rules/page-constraints/`) (the post-selection validators — the Variety Principle (alternation), eyebrow deny-by-default — citing `rules/page-constraints/`). Summaries, not rule tables.
