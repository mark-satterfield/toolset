---
name: export-design
description: Produces one DESIGN.md file — the map of the live Customizable Design System, written to the emerging DESIGN.md convention so any consumer (a human, or a tool that reads DESIGN.md the way AGENTS.md is read for agent instructions) can follow the system without opening the plugin. Emits the color system (palettes → roles → themes with real values), typography, geometry and motion summaries, the Building Blocks catalog (Components, Shapes, Sections, Section Containers, Shells with their contracts and aliases), the rule summaries, the compliance essentials, and how to consume the class and token names. Trigger on "export the design system", "generate DESIGN.md", "give me the design doc", "write the design system file", or "produce a DESIGN.md". The file is regenerated, never hand-edited. Must NOT trigger on stylesheet regeneration (that is generate-stylesheets — its output is an INPUT here), on composing pages/mocks/surfaces (compose-page, compose-app-surface), or on auditing (audit-against-system).
...
1. **Refresh the stylesheet set (freshness stage, cited not restated).** Run the same stylesheet-freshness step the composers run — compare the `cds_hash.py` semantic fingerprints (elements YAML, reference tree, extensions tree) against `manifest.json`; on mismatch invoke `generate-stylesheets` and proceed; **never halt for staleness**; halt `STYLESHEETS_REGEN_FAILED:{inner}` only if that regeneration itself halts, surfacing the inner code verbatim (`../../reference/pipeline.md`, Assemble stage 4). This guarantees the manifest fingerprints written into the stamp match the current inputs.
...
4. **Enumerate the catalog.** Walk `libraries/components/`, `libraries/shapes/`, `libraries/sections/`, `libraries/section-containers/`, `libraries/shells/`, `rules/shape-selection/`, `rules/page-constraints/` (reference ∪ extensions, overlay applied). For each entry read its frontmatter for name, id, family, aliases, and its per-kind contract fields. Hardcode no counts or names — walk what the catalog declares.
5. **Compose `DESIGN.md`** with this exact shape:
   ...
   **Frontmatter** (machine-readable tokens, DESIGN.md spec + the generation-stamp extension):
   ...
   motion:        # easing + durations from motion.md §15 (YAML motion override wins)
     <token>: <value>
   components:    # one entry per Component family, its bound role tokens + sizing token refs
     <name>: { ... }
   ---
   ```
   ...
   **Body** — first line a banner, then the sections in this order:
   ...
   - `## Layout & Spacing` — geometry summary: spacing scale, radius scale, section padding, container widths, reading columns, app-shell panes, control/row heights, with values and token names. Cites `foundations/layout.md §11`.
   ...
   - `## Building Blocks` — the CDS catalog, one subsection per kind (`### Components`, `### Shapes`, `### Sections`, `### Section Containers`, `### Shells`). Each entry is one row: name (and id for sections/shells), a purpose one-liner, its aliases, and the load-bearing contract — for Components the five contracts in brief (slots, sizing, behavior, accessibility, token bindings); for Shapes the slots and `self_contained` flag; for Sections the mode and content contract; for Section Containers the ordered Section list and referenced constraints; for Shells the furniture/panes and accepted content. Each row cites its entry path (e.g. `reference/libraries/components/button.md`) for depth — DESIGN.md names and locates, it does not inline every contract.
   - `## Rules` — `### Shape selection` (per-Section: the signals consulted and that a rule chooses an ordered candidate set, citing `rules/shape-selection/`) and `### Page constraints` (the post-selection validators — variety/alternation, eyebrow deny-by-default — citing `rules/page-constraints/`). Summaries, not rule tables.
   ...
   - `## How to consume` — the practical handles: the generated stylesheet files (`tokens.css`, `components.css`, `themes.css`) and how they are loaded; the class-name convention (kebab-case mirroring role/component names, `.ground--N`/`.feature-tile--N`, the `.u-container`/`.u-reading` utilities); the token-name convention (`var(--sp-*)`, `--container-*`, `--surface-*`, …); and the full alias table (user-facing name → internal Building Blocks term) from `reference/aliases.md` plus any project `aliases.md`.
...
- `STYLESHEETS_REGEN_FAILED:{inner}` — the freshness stage found the stylesheet set stale or missing and the auto-invoked `generate-stylesheets` itself halted; the inner halt code is surfaced verbatim.
...
- Does not generate the stylesheet set — it INVOKES `generate-stylesheets` through the freshness stage when inputs have moved, then reads the manifest. It never authors CSS.
- Does not compose pages, mocks, or app surfaces, and emits no sidecars or state record.
