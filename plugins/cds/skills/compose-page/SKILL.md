...
2. **Section sequence + shape derivation.** For **landing-page requests**, consult `./reference/landing-sections-shape-rules.md` and derive the ordered section sequence + a shape per section using its `pick_shape` rules, variety principle, and adjacency rules. **For any non-landing page type — Editorial Detail, Resource Index, Documentation, Conversion/Authentication, Application Shell — STOP `SHAPE_RULES_PENDING:{page-type}` and surface the gap.** Beta-1 supports landing only.
...
- `SHAPE_RULES_PENDING:{page-type}` — non-landing page type requested; shape rules deferred past Beta-1.
