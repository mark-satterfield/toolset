...
- **From `$CUSTOMIZABLE_DESIGN_SYSTEM_EXTENSIONS_DIR`:** the project extensions directory, mirroring `libraries/` + `rules/` (Library resolution, `../../reference/pipeline.md`).
...
3. **Page and page family.** Resolve the Page: a named library Page entry (e.g. `primary-landing`, `documentation`), or an ad-hoc Page the caller describes Section by Section. Every Page carries a **page family** (landing, app, editorial, docs, auth): take it when stated plainly or obvious from the prompt (a named library Page carries its own `page_family:`); otherwise ASK — never infer silently. A word offered as the family that is not one of the five is not a page family; ask which of the five it is (`../../reference/pipeline.md`, Resolve the Page). The page family selects the typography and motion register and scopes the PageLevelAestheticConstraints.
...
Execute the shared build pipeline (`../../reference/pipeline.md`) with the resolved render target. The pipeline defines library resolution, the Page → per-Section resolution stages (eager and lazy Shape assignment, the PageLevelAestheticConstraints rejection loop, and the four-rung Shape-assignment waterfall that searches the rest of the ShapeLibrary — unmodified, then adapted — before anything is generated from scratch), the silent stylesheet-freshness stage, run-modes, sidecar emission, and the state record. This skill adds only the Page HTML render specifics:
