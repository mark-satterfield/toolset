...
2. **Section sequence + shape derivation.** For **landing-page requests**, consult `./reference/landing-sections-shape-rules.md` and derive the ordered section sequence + a shape per section using its `pick_shape` and adjacency rules, applying the Variety Principle and the background-color alternation schedule from `../../reference/page-types.md` §20 (Primary Landing Page). **For any page type whose shape rules are not present in the reference, STOP `SHAPE_RULES_PENDING:{page-type}` and surface the gap — do not guess.**
