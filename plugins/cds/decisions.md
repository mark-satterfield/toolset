...
**Decision.** Naming a Page or a Section the catalog has not seen composes normally. The composer builds it from its attributes — where it pins, its extent, its theme — and resolves its Shape through the Shape-assignment waterfall. The halt codes `SECTION_TYPE_UNKNOWN` and `PAGE_UNKNOWN` are retired.

The catalog boundary — where an unknown name halts instead of being guessed at — now sits at two places only:
...
**Rejected alternative.** Requiring every Page and Section to have a catalog entry, halting otherwise. That was the behaviour from 2026-07-06 (`36f2856`) until this change.
...
The cost was concrete: a Shell could not put a rail on the inline-end edge without a new catalog entry first, which is a placement decision the catalog has no business owning.
...
## A pinned Section's edge is chosen by the Shell, not by the catalog
...
**Rejected alternative.** Adding a second `right-rail` entry beside `left-rail`. That duplicates a surface contract of roughly twenty determinations to express one attribute, and leaves placement in the catalog.
