...
Run the environment check. It resolves the active interpreter, detects the
package manager, and reports whether an install is needed.
...
- `ACTION: none` — dependencies present. Continue to Step 2.
- `ACTION: <install command>` — run that exact command, then continue.
- `ACTION: stop` — no active virtualenv. Create/activate one (commands are
  printed, and in `references/install.md`), then re-run the check.

Never install into system or Homebrew Python. See references/install.md for the
manager matrix and format-specific installs.
