...
PRD-to-Spec pipeline, phase 2b — turns the PRD's *what* and the ruled architecture decision into the *how*, bounded by the current arc42 SAD, carrying both the PRD requirements needing technical elaboration and the technical requirements the architecture imposes with no PRD parent; maker-checker loop feeds Gate 2b. 6 agents.
...
| `trd-author` | TRD Authoring | Execution Team | Worker | Executor | execute | Authors the TRD — the blueprint for HOW the feature is built — from the PRD requirements needing technical elaboration AND the technical requirements the architecture imposes that no PRD would state; NFR derivations and interface and data obligations bounded by the SAD extract | subagent-contract, validation-protocol, senior-architect | Read, Write, Edit, Glob, Grep, Bash |
