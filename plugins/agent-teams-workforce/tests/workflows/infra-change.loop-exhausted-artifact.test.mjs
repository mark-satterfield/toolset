...
//
// Observation seam: the composite's non-ok return used to expose the gateLoop result as
// `detail`. It no longer does — composites hand their phase detail to the RUN JOURNAL and
// return a path, because returning it truncated single runs and killed the dispatching
// session over a campaign. The shape is therefore asserted against the journalled payload.
// One consequence is stated plainly rather than worked around: the journal is JSON, so
// OBJECT IDENTITY is not observable through this seam and the artifact assertions below
// are structural. `result.intent` still crosses by identity and D4-AC2 still tests that.
...
async function runInfra({ g1Verdicts, intentReturns }) {
  ...
  let g1Count = 0
  const { result, calls } = await runWorkflowScript(INFRA_JS, {
    ...
    workflowImpl: (call) => {
      ...
      if (call.name === 'agent-teams-workforce:workspace') {
        return { ok: true, repoPath: '/tmp/worktrees/fixture', branch: 'infra/ssbd-fixture', reused: false, isLinkedWorktree: true, independentlyVerified: true, defaultBranch: 'main' }
      }
  ...
  // `.g1Loop` is where the G1 exit journals its gateLoop result; the other exits journal
  // the gateLoop result directly.
  const journalled = journalDetail(calls)
  return { result, intentPayloads, detail: (journalled && journalled.g1Loop) || journalled }
}
...
test('D4-AC1: the loop-exhausted gateLoop result carries `artifact` === the final phaseFn return', async () => {
  ...
  const { result, detail } = await runInfra({
    g1Verdicts: LOOP3,
    intentReturns: [
  ...
  assert.deepEqual(
    ...
    'artifact must be the value returned by the FINAL phaseFn call, not an earlier attempt\'s'
  )
...
test('D4-AC4: the no-verdict and escalate exits still carry artifact === the attempt-1 phaseFn result', async () => {
  ...
  assert.deepEqual(dNo.artifact, A1, 'no-verdict exit must carry artifact === attempt-1 phaseFn result')
