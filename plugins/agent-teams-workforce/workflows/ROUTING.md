...
|---|---|---|
| `prd-to-spec` | epic-lifecycle → prd-validation → architecture? → repo-scoping → trd-authoring → (prd-reconciliation → spec-authoring, per repo) → task-decomposition → cross-Story Task dependencies → emit-beads → Epic scoring | a feature with no implementation-ready contract yet. It elaborates an existing Epic and refuses, with a named reason, one that is not open, not scored, waiting on an Epic whose elaboration is not `done`, or not `ready` or `in_progress` with no other owner. The run specifies the WHOLE PRD — the PRD is canonical, and nothing already built ever subtracts from what it asks for. THE THREE LAYERS ARE BLIND TO DIFFERENT THINGS ON PURPOSE: a PRD is WHAT and never knows what is deployed; a TRD is HOW, derived from the PRD and the SAD by expert architecture and best practice, and blind to deployed state too; the SPEC is the only layer that asks "X is what we want, Y is what we have, how do we turn Y into X", because it is the only layer scoped to ONE repository — the only scope at which that question has a concrete answer. So `prd-reconciliation` runs PER REPO INSIDE spec authoring, and nowhere earlier: it is an INVENTORY, not a filter, reporting for every requirement what material exists in that repository and whether it conforms, so the spec knows what to REUSE and what to REMOVE. Where the PRD and what exists disagree, the PRD wins — that is settled by definition and generates removal work, never an architecture question — and removal, DISCOVERED AT SPEC TIME, reaches task decomposition alongside the build work. A repository whose comparison cannot be established gets NO spec: specifying blind would re-specify working material and leave contradicting material standing. `architecture?` is CONDITIONAL — a read-only triage over THE PRD ITSELF runs first and the panel convenes only when the PRD forces a choice between options whose consequences outlive the feature; a difference from what is deployed is never one (the PRD wins by definition) and a UI/UX difference never is either (the design-system artifacts settle it). Otherwise the phase is skipped and the skip recorded with its reason. The repo span is RULED by `repo-scoping` after the architecture decision — it is an output of the run, recomputed every time and never pre-staged; the polyrepo-steward maps the work units onto existing repositories and creates any repository the work needs that the project does not have — a needed repository is never a human action. The final phase WRITES the Story → Task hierarchy beneath the Epic into beads itself, parent before child, each Task carrying every WSJF component and its build dependencies as Task-to-Task `blocks` edges, then scores the Epic and its Tasks and marks the Epic `done`, and reports what landed on `emissionOk` / `beadsEmitted` / `emission` — the caller does not run `bd` for it. It also RETIRES the stand-in roll-up Stories the Epic was carrying: a Task that reached the build lane parentless had one minted for it on the side, and once this run authors the Spec-backed Story that work belongs under, the stand-in's Tasks are re-parented onto a real Story and it is closed. That repair is reported on `emission.heal` and never moves the emission verdict |
| `task-to-deploy` | red → green → refactor → integration → adversarial → deploy | work whose spec/contract already exists and is implementation-ready — the Task's build contract names its repository, spec documents and SAD decision ids, and the run builds to it without ruling any architecture of its own |
