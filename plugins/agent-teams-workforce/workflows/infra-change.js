...
// args: {
//   bead: { id, title, description, repoPath?,    // the infra change bead. repoPath names the
//           repoHints?, prdPath?, specPath?, epic? }, // REPOSITORY when the caller knows it; absent,
//                                                 // the Repo Resolution phase rules it at run time.
//                                                 // The tree the phases write in comes from the
...
async function gateLoop({ gate, phaseName, criteria, checks, escalateTargets, phaseFn, gateWorkflow, initialFeedback }) {
  ...
  const ruling = measuredFailures.length ? null : await ruleExhaustion({ gate, phaseName, artifact, verdict: lastVerdict, unmetCriteria: exhaustedUnmet })
