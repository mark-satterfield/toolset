...
- **Portfolio** — three workflows over the Epic portfolio, each runnable alone, sharing `../scripts/portfolio/depscore.py`:
  ...
  - `seed-portfolio.js` seeds the portfolio once: it runs `dependency-assessment.js` for every open Epic not assessed since the seeding began, one Epic after another in id order, each with `score: false`, then one `wsjf-scoring.js` run with `all` and `rejudge`. A seeding is resumed by passing the same `since`.
