...
- **Portfolio** — four workflows over the Epics and Tasks, each runnable alone, sharing `../scripts/portfolio/depscore.py`:
  ...
  - `seed-portfolio.js` seeds the portfolio's architecture dependencies, one batch per run: it runs `dependency-assessment.js` for at most `batch` open Epics (default 40, at most 100) not assessed since the seeding began, one Epic after another in id order, each with `score: false`, returns the Epics still to assess in `remaining`, and never scores. `/seed-portfolio` runs it again with the same `since` until `remaining` is empty, then runs `wsjf-scoring.js` with `all` and `rejudge` as a workflow of its own. An assessment that stops on a proposal that never validated stops the seeding at that Epic, with its findings; a seeding is resumed by passing the same `since`.
