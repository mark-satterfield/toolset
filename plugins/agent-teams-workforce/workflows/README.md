...
- **Portfolio** — four workflows over the Epics and Tasks, each runnable alone, sharing `../scripts/portfolio/depscore.py`:
  ...
  - `seed-portfolio.js` seeds the portfolio once: it runs `dependency-assessment.js` for every open Epic not assessed since the seeding began, one Epic after another in id order, each with `score: false`, then one `wsjf-scoring.js` run with `all` and `rejudge`. An assessment that stops on a proposal that never validated stops the seeding at that Epic, with its findings; a seeding is resumed by passing the same `since`.
