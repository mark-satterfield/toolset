...
- **Portfolio** — three workflows over the Epic portfolio, each runnable alone, sharing `../scripts/portfolio/depscore.py`:
  ...
  - `dependency-assessment.js` writes Epic dependency edges — architecture dependencies: which requirements an architecture decision is designed from first — and nothing else — for one new or changed Epic, confined to that Epic's edges, or for the whole portfolio — then triggers `wsjf-scoring.js`.
