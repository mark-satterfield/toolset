...
- **Portfolio** — four workflows over the Epics and Tasks, each runnable alone, sharing `../scripts/portfolio/depscore.py`:
  ...
  - `seed-portfolio.js` seeds the portfolio's architecture dependencies: it runs `dependency-assessment.js` for every Epic `/seed-portfolio` lists as not assessed since the seeding began, one Epic after another in id order, each with `score: false`, and never scores. The first assessment writes the PRD corpus and the rest read it. `/seed-portfolio` checks from the tracker that nothing is left, then runs `wsjf-scoring.js` with `all` and `rejudge` as a workflow of its own. An assessment that stops stops the seeding at that Epic, with its findings; a seeding is resumed by passing the same `since`.
