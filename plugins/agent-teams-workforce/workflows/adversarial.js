...
if (contractPathFault) {
  ...
  return {
    ...
    attackers: [],
    laneMode: 'refused',
    ledger: { phase: 'adversarial', beadId: (c.bead && c.bead.id) || null, chosen: [], mode: 'refused', ok: false },
  }
}
...
if (unruled.length) {
  ...
  // A follow-up that DIED ruled on nothing, so the findings it was asked about are unruled
  // because a dispatch failed, not because the adjudicator left them open. Counting them as
  // open constitutive findings would fail Gate 4 — charged to the bead — on a dead session,
  // so the phase reports the death the way a dead first adjudication is reported.
  if (!followUp) {
    const reason = `the adversarial-critique-adjudicator follow-up returned nothing — ${unruled.length} confirmed finding(s) were never adjudicated`
    log(`Adversarial: ${reason}`)
    return {
      dispatchFailed: true,
      dispatchFailures: dispatchDeaths('Adjudicate'),
  ...
  for (const r of Array.isArray(followUp.rulings) ? followUp.rulings : []) {
