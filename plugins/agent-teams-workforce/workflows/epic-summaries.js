...
Settled: for EVERY decision named under Designed on, one short clause: settled by the SAD, citing the section or file, or open. Then the same for the decisions under Drives. A decision the SAD settles needs no design order, so the reader decides edges from this field. Report what is decided, not the design itself: cite the section, and do not restate its content. Write "open" for a decision the searches found nothing on.
