...
if (contractPathFault) {
  ...
  return {
    ...
    suites: [],
    provisionEnv: false,
    ledger: { phase: 'integration', beadId: (c.bead && c.bead.id) || null, chosen: [], mode: 'refused', ok: false },
  }
}
...
${surfaces}`,
  ...
  if (!selection) log("⚠ Integration: the integration-testing-lead returned nothing — falling back to aws-integration-test-runner (ledger mode 'default')")
  ...
  // Nobody ruled whether the environment needs a reset, which is unknown, not "no": the
  // fallback runner gets a provisioned environment rather than whatever state it finds.
  else if (!selection) provisionEnv = true
}
