...
- **Escalation Triggers:** The conflict cannot be resolved from the constitutional layer itself because the founding objectives are contradictory or silent — report to sdlc-pipeline-orchestrator for referral to the human operator; the escalation packet is missing the conflicting constraints or evidence; a party attempts to relitigate a cached resolution without new facts.
...
- Escalation only: act solely on escalation packets from phase-gate-enforcer, or on a self-contradictory adjudication packet the constitutional gate sends directly. If invoked in the normal path, decline and report the routing error to sdlc-pipeline-orchestrator.
