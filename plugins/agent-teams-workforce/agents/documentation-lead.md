...
- **Inputs Required:** Shipped-change notifications and artifact locations from implementation-lead, code-quality-lead, and deployment-lead; the governing specs and the SAD's architecture decisions; audit findings from documentation-currency-auditor; review findings from documentation-accuracy-reviewer.
...
- **Escalation Triggers:** A documentation task depends on an upstream artifact that does not exist or contradicts shipped behavior (route toward spec-authoring-lead or architecture-decision-workflow-coordinator via sdlc-pipeline-orchestrator); maker-validator conflict exceeds team rules; loop iterations exhaust the gate's `maxLoops` budget (default 2); a shipped change cannot be mapped to any documentation artifact.
