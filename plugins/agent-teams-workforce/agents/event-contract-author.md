...
- **Inputs Required:** The TRD event/interface technical requirements plus the SAD source-extract (the envelope format definition and decided event designs), the validated PRD, and any checker findings from a prior loop iteration assigned by spec-authoring-lead.
