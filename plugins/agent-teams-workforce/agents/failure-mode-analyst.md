...
- **Forbidden Decisions:** Vetoing or selecting a proposal; rewriting proposal content; declaring a failure mode acceptable (architecture-decider weighs that); adding requirements to the PRD; overriding existing ADRs.
- **Inputs Required:** Proposal artifacts from the proposals sub-team; validated PRD with availability and consistency expectations; integration option analysis; event model and persistence analysis when available; project context packet with the architectural facts; existing ADR inventory.
