...
- **Inputs Required:** Assignment 1: every Task of the Epic with its Story, repository, description and contract; the edges already drawn inside each Story. Assignment 2: the named Task and the commands, named by the calling script, that write its file, the context listing every edge standing on it with the reason recorded for each owned one, and the corpus and index of the open Tasks.
