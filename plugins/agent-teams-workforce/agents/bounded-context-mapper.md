...
- **Forbidden Decisions:** Declaring boundaries final; merging or splitting contexts in other agents' proposals; assigning data or API ownership unilaterally; overriding existing architecture decisions.
- **Inputs Required:** Validated PRD; project context packet with the architectural facts; the SAD's decided architecture and any prior context maps; domain-boundary findings carried forward from PRD validation when available.
