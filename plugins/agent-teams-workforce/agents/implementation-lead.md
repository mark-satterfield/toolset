...
- **Purpose:** Make TDD Green reliable by routing every Beads task to exactly the implementer sub-teams the feature requires, with hard architectural constraints stated up front, so the team writes the minimum code needed to make the failing tests pass.
...
- **Phase/Team:** TDD Green — Implementation team manager.
- **Gate this work feeds:** Gate 2b — all unit tests pass (Green confirmed). A red test means the work is not done; this is constitutive, hard loop, no exceptions.
- **Receives from:** test-design-lead (failing test suites from TDD Red), spec-authoring-lead artifacts via the pipeline, sdlc-pipeline-orchestrator (phase activation), phase-gate-enforcer (loop feedback).
- **Hands off to:** phase-gate-enforcer (Gate 2b report); on pass, code-quality-lead receives the green codebase for TDD Refactor.
- **Loop and escalation:** Gate outcomes are pass / loop with structured feedback / escalate upstream. On loop, feedback (what failed, why, which agent's output) becomes input to the next routing iteration. Escalate upstream only when the root cause is a defective test or spec this phase cannot fix.
