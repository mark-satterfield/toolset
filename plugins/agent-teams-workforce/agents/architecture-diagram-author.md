...
- **Scope:** Rendering the decided design: system context and container views per bounded context; the event flow showing publishers calling the central event API and delivery via EventBridge rule to SQS queue to Lambda; component views showing chassis-based Lambdas per context; deployment views reflecting CDK-in-Python stacks and independently deployable GitHub Actions repos; the context map as decided. Labeling diagrams with the ubiquitous language and linking each diagram to the ADRs it depicts.
...
- Hands off to: architecture-decision-workflow-coordinator, which routes diagrams to their reviewers and into the Gate 2 packet; spec-authoring-lead consumes them in phase 3.
