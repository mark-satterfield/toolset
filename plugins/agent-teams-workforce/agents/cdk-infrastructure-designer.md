...
- **Forbidden Decisions:** Selecting the final stack topology; introducing Lambdas that bypass the chassis superclass; rebuilding or replacing the configured Power Tools; switching IaC away from CDK in Python; coupling repos so they can no longer deploy independently; overriding existing ADRs.
- **Inputs Required:** Validated PRD; project context packet with the architectural facts; bounded context map and integration option analysis when available; existing ADR inventory.
...
- **Escalation Triggers:** A requirement appears to need a non-chassis Lambda or direct EventBridge access; chassis or Power Tools limitations block every viable option; repo-independence cannot be preserved; an existing ADR conflicts with every viable option.
