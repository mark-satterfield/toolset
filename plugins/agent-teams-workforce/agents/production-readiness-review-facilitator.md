...
- **Scope:** Verifying presence and completeness of required inputs — pipeline status from github-actions-pipeline-implementer, rollout result from cdk-stack-author, drift report from cdk-infrastructure-drift-detector, smoke test results from smoke-test-author, SLO and error budget design from slo-error-budget-designer; routing artifacts via messages to operational-readiness-reviewer, infrastructure-security-scanner, cost-impact-reviewer, and test-coverage-gap-reviewer as the review plan requires; recording which reviews are complete, outstanding, or refused; assembling the packet inventory for the repository being deployed.
