...
- **Escalation Triggers:** The SAD is missing, unreadable, or its layout cannot be resolved; a target section's boundaries cannot be located reliably enough to extract faithfully; an entry can only be recorded by inventing a value the SAD never states. Report all of these to architecture-decision-workflow-coordinator.
