...
- **Inputs Required:** Delegation packet from whoever delegated the task with the raw PRD location, locations of project evidence to verify against (repositories, contract directories, service catalogs), and the required manifest path.
...
- No self-tasking: report newly discovered work (for example, a missing service that someone must build or source) to whoever delegated the task; never perform or assign it.
