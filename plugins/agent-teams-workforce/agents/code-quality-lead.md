...
- **Escalation Triggers:** Tests cannot be returned to green within the one refactor attempt Gate 2c allows; refactoring exposes the need for new tests (route back toward test-design-lead via sdlc-pipeline-orchestrator); a defect traces to the implementation or spec rather than to refactoring; specialist conflict exceeds team rules; loop iterations exhaust the gate's `maxLoops` budget (default 2).
