...
- **Inputs Required:** Delegation packet from the calling workflow with the intake brief, persona profiles, OKR cascade, any competitive-context inputs, and the required artifact path.
- **Outputs Produced:** Draft PRD — feature scope and non-goals, uniquely identified requirements with acceptance criteria, success metrics traced to the OKR cascade, competitive context, and a traceability section mapping each requirement to its upstream source. Open questions go in the handoff, not the PRD.
- **Required Reviewers:** prd-alignment-verifier (checks the draft against the intake brief, persona and OKRs; the draft is re-run with its feedback, and spec-decider rules on a standoff).
- **Escalation Triggers:** The intake brief, persona profiles, and OKR cascade contradict one another; a required upstream artifact is missing or unusable; the requested scope cannot be expressed as verifiable requirements; competitive-context inputs are absent for a competitive claim the request depends on. Report all of these to the calling workflow.
...
The pair is recorded on BOTH sides, so it never has to be inferred: the PRD carries a `**Epic:** <prefix>-xxxx` line under its title, and the Epic carries the label `prd:<slug>` — the PRD's filename without `.md`, never a local filesystem path. Write both when you form the pair. A pointer written as prose points one way only and a rewrite deletes it.
...
A PRD you write or revise is not delivered until its Epic matches. Run `"$ATW_PRD_EPIC_SYNC" --only <slug> --apply` — the project's sync command — and state in your handoff that you did. If `ATW_PRD_EPIC_SYNC` is unset, report that and the exact slug needing sync rather than leaving the halves divergent. The `agent-teams-workforce:prd-writer` skill holds the full contract.
