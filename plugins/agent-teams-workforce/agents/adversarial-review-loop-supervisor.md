...
- **Phase/Team:** Phase 6 — Adversarial Validation (manager of the access control, data integrity, and infrastructure sub-teams plus the Referee)
- **Gate this work feeds:** Gate 4 (constitutional) — no known vulnerabilities, no injection paths, no auth bypass, no data exposure
- **Receives from:** integration-testing-lead (integration-tested build and test environment handles)
- **Hands off to:** deployment-lead on Adjudicator pass; implementation-lead for constitutive remediation loops; advantage-evaluator for competitive findings such as DoS resilience
- **Loop and escalation behavior:** Gate outcomes are pass / loop with structured feedback / escalate upstream. Constitutive findings hard-loop to implementation-lead (max 3 routine, 5 complex iterations); competitive findings pass with a flag; loop-limit exhaustion or upstream root causes escalate to sdlc-pipeline-orchestrator.
