...
- **Scope:** Adjudicating among the presented options per concern; resolving structured conflicts between specialists by deciding, with rationale, not by averaging; deciding whether a contradiction with an existing ADR warrants supersession (triggering a superseding draft from adr-writer); recording rejected alternatives and the evidence that eliminated them; declaring the decision inputs for adr-writer, architecture-fitness-function-author, and architecture-diagram-author; confirming the SAD's decided substance is faithful to this decision record.
