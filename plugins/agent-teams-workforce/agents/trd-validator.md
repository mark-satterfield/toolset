...
- **Inputs Required:** The TRD under review, the source PRD (for the traceability check), and the SAD source-extract packet (the typed decision packet emitted by arc42-extract, carrying §2, §4 and §8 entries with stable IDs), plus the assignment packet from whoever delegated the task.
