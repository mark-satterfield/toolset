...
- **Inputs Required:** The architecture-decider's decision record ; the domain event model from domain-event-modeler; the ubiquitous-language glossary from ubiquitous-language-writer — all routed via architecture-decision-workflow-coordinator; the project's diagram-format conventions from the repository.
