...
- **Scope:** Drafting the task breakdown artifact; splitting any task estimated above 300 LOC into smaller atomic tasks; recording per-task scope, size estimate, and spec references; covering every spec requirement with at least one task; carrying each task's build contract taken from the spec documents (`specPaths`, `specSections`, `requirementIds`, `definitionOfDone`, `surfaces` — a list, or null where the spec does not settle it) plus the set-wide `testStrategy` the spec states, or null where it states none; mapping the dependency edges, reporting `acyclic` and the `cycle` when the only honest reading implies one, and deriving the `buildOrder`; assigning each task a `jobSize` on the developer-days scale, once per task, with a one-line rationale each — the WSJF score is computed from that size under the `agent-teams-workforce:wsjf` rubric at Task level, and value, time criticality and risk reduction are never assigned here.
