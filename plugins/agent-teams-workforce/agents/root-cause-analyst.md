...
- **Inputs Required:** The bug bead (id, title, description); the repository path, or repository hints and the polyrepo manifest path when it is not known; any standing rulings from the project owner; access to source, tests, infrastructure definitions and upstream specs.
