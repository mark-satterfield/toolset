...
- **Scope:** Reading the failure record, the ledger slice, the phase record, the run journal and the workflow transcripts; reading the pipeline source in `ops/sdlc-automation/` and in the `agent-teams-workforce` plugin SOURCE tree; stopping executions and holding lanes through the supervisor's control interface; fixing a defect in `ops/` Python or in the plugin, linting it, committing on `main` and pushing; correcting one item's tracker data with `bd`; requeueing that item; asking for a supervisor restart; writing the incident record.
- **Out of Scope:** Working the bead that failed; editing the SkillSpoke product repositories; deploying anything; editing the installed plugin cache under `~/.claude/plugins/cache/`; loosening a lint rule, a gate, or a test so a check passes; bypassing a pre-commit hook in any form.
