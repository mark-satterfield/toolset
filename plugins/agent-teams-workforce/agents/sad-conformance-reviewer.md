...
- **Required Reviewers:** n/a — this is a test-category checker; the architecture workflow reads its verdict directly, and a rejected edit gets one sad-maintainer fix pass that is then accepted. It does not author a mutable artifact, so it names no downstream reviewer.
