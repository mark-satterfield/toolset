...
| 9 — Architecture Decisions | `decisions-index` | The current index of binding decisions, each linking its governing ADR | **TRD author** and **spec authors** (follow links to find the rationale behind a constraint or strategy) |

## Direction of the feed

```
                        SAD (arc42)
                 §2  §4  §8  §9  (SOURCE sections only)
                          │
                  arc42-extract
                          │
            ┌─────────────┴──────────────┐
...
- **Current-state payload.** Because the SAD is a living document (see `living-document-rules.md`), every extraction returns present truth. There is no historical payload; rationale and rejected alternatives are reached by following the ADR links in `decisions-index`, not by reading SAD prose.
